...
```
src/patchwork/algorithm.ts   the instrumented port — one function, ~400 lines
src/patchwork/params.ts      every parameter, with the reference values
src/patchwork/czm.ts         Concentric Zone Model geometry
src/core/linalg.ts           closed-form symmetric 3x3 eigensolver + PCA plane fit
src/stages/                  the twelve tutorial stages
...
plane and accepted set, the GLE verdict with all five measured quantities, and the TGR
probabilities. The visualisation reads that trace; it never recomputes anything.

### Verification

The port is checked against an independent NumPy implementation
(`reference/patchworkpp_prototype.py`) of the same reference C++:

```
node --experimental-strip-types scripts/verify-port.ts
```

Both agree exactly on ground/non-ground counts, RNR removals, fitted cells, definite-ground
cells, TGR candidates and reverts, across all six sample frames.
